...
import type { GameResolveResponse } from "@singularities/shared";
...
interface GameResultProps {
  result: GameResolveResponse;
  onDone: () => void;
}

export function GameResultDisplay({ result, onDone }: GameResultProps) {
  ...
  }, [result.score, result.detected, result.levelUp]);
    ...
    <div className="space-y-4">
      ...
      >
        <div className="text-sm font-bold">
          {result.score >= 50 ? "INFILTRATION SUCCESS" : "INFILTRATION FAILED"}
        </div>
        {result.detected && (
          <div className="text-xs text-cyber-red">DETECTED</div>
        )}
      </motion.div>
      ...
      {/* Terminal output */}
      <div className="bg-bg-primary border border-border-default rounded p-3 font-mono text-xs leading-relaxed min-h-[80px]">
